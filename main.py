    ...
    async def _process_single_url(
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"抓取网页失败 (静默跳过): {url}, 错误: {e}")
                return None # 修改：出错直接返回 None

            if not html:
                logger.warning(f"网页内容为空 (静默跳过): {url}")
                return None
            ...
            try:
                ...
                logger.warning(f"解析网页失败 (静默跳过): {url}, 错误: {e}")
                return None # 修改：解析失败返回 None

            if not content_data or not content_data.get("content"):
                logger.warning(f"提取内容为空 (静默跳过): {url}")
                return None
            ...
            try:
                if self.enable_translation:
                    try:
                        ...
                        logger.warning(f"翻译失败，尝试使用原始内容: {url}, 错误: {e}")
                        analysis_result = await self.analyze_with_llm(event, content_data)
                else:
                    analysis_result = await self.analyze_with_llm(event, content_data)
            except Exception as e:
                logger.error(f"LLM分析失败 (静默跳过): {url}, 错误: {e}")
                return None # 修改：分析失败返回 None
            ...
            if self.enable_screenshot and self.send_content_type != "analysis_only":
                try:
                    ...
                    logger.warning(f"截图失败: {url}, 错误: {e}")
            ...
            # 捕获所有未预料的异常，静默跳过
            logger.error(f"处理URL发生未知错误 (静默跳过): {url}, 错误: {e}")
            return None
